function initUI() {
    ...
    urgentButton.addEventListener('click', ()=>sendWebhookMessage(urgentMessage.replace('{time}', new Date().toLocaleString())));
    warningButton.addEventListener('click', ()=>sendWebhookMessage(warningMessage.replace('{time}', new Date().toLocaleString())));
...
document.addEventListener('DOMContentLoaded', () => {
    if (document.getElementById('lanis-ui')) return; // 이미 있으면 종료
    initUI(); // UI 생성 함수
});
